...
class SolanaClass {
    constructor(provider, renNetwork, options) {
        ...
        this.submitMint = async (asset, contractCalls, mintTx, eventEmitter) => {
            ...
            const recipientTokenAccount = new web3_js_1.PublicKey(contractCalls[0].sendTo);
            ...
            const simulationResult = await utils_1.retryNTimes(async () => this.provider.connection.simulateTransaction(tx), 5);
            ...
            const confirmedSignature = await web3_js_1.sendAndConfirmRawTransaction(this.provider.connection, signed.serialize(), { commitment: "confirmed" });
            // FIXME: this follows eth's events, generalize this
            eventEmitter.emit("confirmation", 1, { status: 1 });
            // Wait up to 20 seconds for the transaction to be finalized.
            await util_1.finalizeTransaction(this.provider.connection, confirmedSignature);
        ...
        this.getMintParams = async (asset) => {
            ...
            const recipient = params && params.contractCalls
                ? new web3_js_1.PublicKey(params.contractCalls[0].sendTo)
                : this.provider.wallet.publicKey;
            const destination = await utils_1.retryNTimes(async () => this.getAssociatedTokenAccount(asset, recipient.toString()), 5, 3 * utils_1.SECONDS);
            if (!destination) {
        ...
        this.findBurn = async (asset, eventEmitter, _transaction, burnNonce) => {
            ...
            if (typeof burnNonce == "number") {
                leNonce = new bn_js_1.BN(burnNonce).toBuffer("le", 8);
            }
        ...
        this.submitBurn = async (asset, eventEmitter, contractCalls) => {
            ...
            const confirmedSignature = await web3_js_1.sendAndConfirmRawTransaction(this.provider.connection, signed.serialize(), confirmOpts);
            ...
            eventEmitter.emit("transactionHash", bs58_1.default.encode(signed.signature));
            // Wait up to 20 seconds for the transaction to be finalized.
            await util_1.finalizeTransaction(this.provider.connection, confirmedSignature);
            const x = {
    ...
    async getAssociatedTokenAccount(asset, address) {
        ...
        const targetAddress = address
            ...
            : this.provider.wallet.publicKey;
        const tokenMintId = await this.getSPLTokenPubkey(asset);
        const destination = await associated_token_1.getAssociatedTokenAddress(targetAddress, tokenMintId);
        try {
    ...
    async createAssociatedTokenAccount(asset, address) {
        ...
        if (!existingTokenAccount) {
            const createTxInstruction = await associated_token_1.createAssociatedTokenAccount(this.provider.wallet.publicKey, targetAddress, tokenMintId);
            ...
            try {
                // Should be the same as `signature`.
                const confirmedSignature = await web3_js_1.sendAndConfirmRawTransaction(this.provider.connection, signedTx.serialize(), confirmOpts);
                // Wait up to 20 seconds for the transaction to be finalized.
                await util_1.finalizeTransaction(this.provider.connection, confirmedSignature);
                return confirmedSignature;
            }
